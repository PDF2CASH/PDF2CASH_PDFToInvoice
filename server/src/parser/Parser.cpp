...
void resetHTMLOutputVariables()
{
    ...
    stout=gFalse;
    xml=gFalse;
    noRoundedCoordinates = gFalse;
    errQuiet=gFalse;
    noDrm=gFalse;
    wordBreakThreshold=10;  // 10%, below converted into a coefficient - 0.1
...
bool Parser::ProcessPDF(QString pdfFileName)
{
    ...
    if (doc == nullptr || !doc->isOk())
    {
        ...
        doc = doc->ErrorPDFDoc(doc->getErrorCode(), fileName);
        if (doc == nullptr || !doc->isOk())
        {
            //PDFDoc doc2(fileName);
